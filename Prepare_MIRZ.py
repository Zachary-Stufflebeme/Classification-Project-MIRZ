...
# DELETE REDUNDANT COLUMNS / DUPLICATE DATA
...
#In the following 13 lines I am encoding my categorical variables so that I can use this dataframe in our classification methods without problems
    ...
    drops = ['gender','partner','dependents','phone_service','paperless_billing','churn', 'internet_service_type',
             'multiple_lines','online_security', 'online_backup', 'device_protection', 'tech_support',
             'streaming_tv','streaming_movies','Unnamed: 0','churn_month','signup_date']
    for x in drops:
        if x in df.columns:
    ...
    #Filling nulls with 0 for items where people dont have internet resulting in them not having other features.
    cols = ['has_mult_lines', 'has_online_security','has_online_backup','has_device_protection','has_tech_support',
            'can_stream_tv','can_stream_movie']
    for x in cols:
        df[x] = df[x].fillna(0)
    ...
    df[['monthly_charges', 'total_charges','tenure']] = scaler.fit_transform(df[['monthly_charges', 'total_charges','tenure']])
